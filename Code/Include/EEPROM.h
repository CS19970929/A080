...
//IO方向设置
//改括号里面那个数X<<1
#define SDA_IN_SEE()  {GPIOB->MODER&=0xFFFFFCFF;GPIOB->MODER|=(UINT32)0<<(4<<1);}		//默认值为00，既不上拉也不下拉输入。
#define SDA_OUT_SEE() {GPIOB->MODER&=0xFFFFFCFF;GPIOB->MODER|=(UINT32)1<<(4<<1);}		//关于输出类型和输出速度，在默认值的Init函数配好，回来立刻生效

//IO操作函数	 
#define IIC_SCL_SEE    PORT_OUT_GPIOB->bit3 	//SCL
#define IIC_SDA_SEE    PORT_OUT_GPIOB->bit4		//SDA
#define READ_SDA_SEE   (uint16_t)(GPIOB->IDR&GPIO_Pin_4)  //输入SDA 
...
#define EEPROM_VALUE_BEGIN_FLAG				0x1138		//默认0x1133，如果自己需要刷一遍，则自己改完再改回0x1133
...
#define E2P_ADDR_E2POS_ENHANCE_SOC		790		//到820
...
#define E2P_ADDR_SOC_RTC_CNT          	1202		//RTC次数统计，用于是否更新SOC
