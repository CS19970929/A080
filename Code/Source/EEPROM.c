...
UINT8 QuickStartUpFlag = 0;
void InitE2PROM_Quick(void) {
	GPIO_InitTypeDef GPIO_InitStructure;
	...
	if(EEPROM_VALUE_BEGIN_FLAG == ReadEEPROM_Word_NoZone(EEPROM_ADDR_PASS)) {//第二次上电就会执行这个
		for(i = 0; i < 2; ++i) {		//Other_CanAdd
			t_u16RdTemp = ReadEEPROM_Word_WithZone((UINT16)*(&OtherElement_to_Pos.u16CBC_DelayT+i));
			t_u16TempMax = (*(&OtherElement_to_Max.u16CBC_DelayT+i));
			t_u16TempMin =  (*(&OtherElement_to_Min.u16CBC_DelayT+i));
			if ((t_u16RdTemp >= t_u16TempMin) && (t_u16RdTemp <= t_u16TempMax)) {
				*(&OtherElement.u16CBC_DelayT+i) = t_u16RdTemp;
			}
			else {
				*(&OtherElement.u16CBC_DelayT+i) = *(&OtherElement_to_default.u16CBC_DelayT+i);
				if(0 == System_ErrFlag.u8ErrFlag_Com_EEPROM) {
					//g_st_SysStatusFlag.bits.b1EepromErr = 1;
					System_ERROR_UserCallback(ERROR_EEPROM_STORE);
				}
		...
		for(i = 0; i < 2; ++i) {		//Other_CanAdd
			t_u16RdTemp = ReadEEPROM_Word_WithZone((UINT16)*(&OtherElement_to_Pos.u16Sys_CS_Res+i));
			t_u16TempMax = (*(&OtherElement_to_Max.u16Sys_CS_Res+i));
			t_u16TempMin =  (*(&OtherElement_to_Min.u16Sys_CS_Res+i));
			if ((t_u16RdTemp >= t_u16TempMin) && (t_u16RdTemp <= t_u16TempMax)) {
				*(&OtherElement.u16Sys_CS_Res+i) = t_u16RdTemp;
			}
			else {
				*(&OtherElement.u16Sys_CS_Res+i) = *(&OtherElement_to_default.u16Sys_CS_Res+i);
				if(0 == System_ErrFlag.u8ErrFlag_Com_EEPROM) {
					//g_st_SysStatusFlag.bits.b1EepromErr = 1;
					System_ERROR_UserCallback(ERROR_EEPROM_STORE);
				}
