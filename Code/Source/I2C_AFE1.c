...
int InitialisebqMaximo(unsigned char I2CSlaveAddress) {
	...
	// Registers_AFE1.SysCtrl2.SysCtrl2Bit.CHG_ON = 1;		//普通情况起来，要初始化数值才能打开管子。
	// Registers_AFE1.SysCtrl2.SysCtrl2Bit.DSG_ON = 1;		//
	...
	if(QuickStartUpFlag) {
		...
		Registers_AFE1.SysCtrl2.SysCtrl2Bit.CHG_ON = 0;		//普通情况起来，要初始化数值才能打开管子。
		Registers_AFE1.SysCtrl2.SysCtrl2Bit.DSG_ON = 0;		//
	}
...
void InitAFE1(void) {
	...
	InitShortCur();
	I2CReadBlockWithCRC(DEVICE_ADDR_AFE1, PROTECT1, &test3, 1);
}
