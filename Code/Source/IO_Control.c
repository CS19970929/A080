...
void App_DI1_Switch(void)
{
...
#if (defined _MOS)
#if 0
	if((0 == MCUI_ENI_DI1) && (g_stCellInfoReport.u16Ichg < 20)) {
	...
	switch (Driver_Element.DriverForceExt.bits.b2_Force_MOS_DSG)
	{
...
#if 1
	static UINT8 su8_DI_status = 0;
	static UINT16 su16_AntiShake_Cnt1 = 0;
	static UINT16 su16_Sleep_Tcnt = 0;

	switch (su8_DI_status)
	{
	case 0:
		if (0 == MCUI_ENI_DI1)
		{
	...
	case 2: // 短按关闭放电MOS
		if (g_stCellInfoReport.u16Ichg < 20)
		{
			Driver_Element.DriverForceExt.bits.b2_Force_MOS_DSG = FORCE_CLOSE_MODE;
		}
		else
	...
	static UINT8 su8_ComFlag = 0;
	static UINT8 su8_ComCnt = 0;
	static UINT8 su8_ComOver = 0;

	if(su8_ComCnt != RTC_ExtComCnt) {
		su8_ComCnt = RTC_ExtComCnt;
		su8_ComOver = 0;
		su8_ComFlag = 1;
	}
	else {
	...
	if(g_stCellInfoReport.u16IDischg > 0 || su8_ComFlag) {
		return;
	}

	if(1 == MCUI_ENI_DI1) {
		if(++su16_AntiShake_Cnt1 >= 20) {
			su16_AntiShake_Cnt1 = 20;
			Sleep_Mode.bits.b1ForceToSleep_L2 = 1;
		}
	}
	else {
