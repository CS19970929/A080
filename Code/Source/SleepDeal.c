...
void SleepDeal_OverCurrent(void) {
	...
	if(!Sleep_Mode.bits.b1OverCurSleep) {		//加强雍余设计
...
void SleepDeal_VcellUVP(void) {
	...
	if(!Sleep_Mode.bits.b1VcellUVP) {		//加强雍余设计
		Sleep_Status = SLEEP_HICCUP_SHIFT;	//其实这个可以不要，设计要求，除了这个函数可以把这个标志位去除外，别的地方不可以便可
		return ;
	}
...
void SleepDeal_Forced(void) {
	...
	if(!Sleep_Mode.bits.b1ForceToSleep_L1 && Sleep_Mode.bits.b1ForceToSleep_L2 && Sleep_Mode.bits.b1ForceToSleep_L3) {	//加强雍余设计
		Sleep_Status = SLEEP_HICCUP_SHIFT;	//其实这个可以不要，设计要求，除了这个函数可以把这个标志位去除外，别的地方不可以便可
		return ;
	}
...
void SleepDeal_CBC(void) {
	...
	if(!Sleep_Mode.bits.b1CBCSleep) {		//加强雍余设计
		Sleep_Status = SLEEP_HICCUP_SHIFT;	//其实这个可以不要，设计要求，除了这个函数可以把这个标志位去除外，别的地方不可以便可
		return ;
	}
...
void SleepDeal_Normal_L2(void) {
	...
	if(g_stCellInfoReport.u16VCellMin < OtherElement.u16Sleep_Vlow\
		|| g_stCellInfoReport.u16VCellMin > OtherElement.u16Sleep_VNormal) {	//触发条件才跳转，别的时间不跳转
		Sleep_Mode.bits.b1NormalSleep_L2 = 0;
		Sleep_Status = SLEEP_HICCUP_SHIFT;
		s_u8SleepStatus = FIRST;
		if(s_u32SleepFirstCnt)s_u32SleepFirstCnt = 0;
...
void SleepDeal_Normal_L3(void) {
	...
	if(g_stCellInfoReport.u16VCellMin >= OtherElement.u16Sleep_Vlow) {	//触发条件才跳转，别的时间不跳转
		Sleep_Mode.bits.b1NormalSleep_L3 = 0;
		Sleep_Status = SLEEP_HICCUP_SHIFT;
		s_u8SleepStatus = FIRST;
		if(s_u32SleepFirstCnt)s_u32SleepFirstCnt = 0;
...
void SleepDeal_Normal_Select(void) {
	...
	if(g_stCellInfoReport.u16Ichg <= OtherElement.u16Sleep_VirCur_Chg\
		&& g_stCellInfoReport.u16IDischg <= OtherElement.u16Sleep_VirCur_Chg) {
		if(g_stCellInfoReport.u16VCellMin < OtherElement.u16Sleep_Vlow) {
...
void SleepDeal_Test(void) {
	...
	if(!Sleep_Mode.bits.b1TestSleep) {	//加强雍余设计
		Sleep_Status = SLEEP_HICCUP_SHIFT;
		return ;
	}
...
void App_SleepDeal(void) {
	if(!System_OnOFF_Func.bits.b1OnOFF_Sleep) {			//有个疑问，是不是立刻关了，不需要复原呢，均衡是需要关掉复原。
	...
	if(0 == g_st_SysTimeFlag.bits.b1Sys1000msFlag1 && !Sleep_Mode.bits.b1ForceToSleep_L1\
		 && !Sleep_Mode.bits.b1ForceToSleep_L2 && !Sleep_Mode.bits.b1ForceToSleep_L3) {
...
void App_NormalSleepTest(void) {
	static UINT16 s_u16HaltTestCnt = 0;
		...
		if(0 == g_st_SysTimeFlag.bits.b1Sys1000msFlag1) {	//休眠起来等待系统初始化完成
		return;
	}

	if(++s_u16HaltTestCnt >= 5) {		//10s——Test
		s_u16HaltTestCnt = 0;
		IOstatus_TestMode();
		InitWakeUp_TestMode();
...
void App_RTCSleepTest(void) {
	...
	if(0 == g_st_SysTimeFlag.bits.b1Sys200msFlag1) {	//休眠起来等待系统初始化完成
		return;
	}

	if(++s_u16HaltTestCnt >= 3) {		//10s——Test
		s_u16HaltTestCnt = 0;
		IOstatus_RTCMode();
		InitWakeUp_RTCMode();
		Sys_StopMode();
